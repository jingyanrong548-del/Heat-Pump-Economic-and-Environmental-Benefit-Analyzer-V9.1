...
// ... existing code ...
                    <span class="text-gray-700 tooltip-container">简单投资回报率 (ROI)
                        ...
                        <span class="tooltip-text" style="width: 320px; margin-left: -160px;"><b>通俗解释:</b> 这是一个简化的、非财务人员常用的参考指标 (公式: 第1年节省成本 / 额外投资)。<br><strong class="text-yellow-300">特别注意:</strong> 此指标未考虑资金的时间价值和未来通胀，仅供初步参考。<b>内部收益率 (IRR, 税后) 是更精确的指标。</b></span>
                    </span>
                    <span class="font-bold ${simpleRoiColor}">${fPercent(boiler.simpleROI)}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <!-- V11.0: 更新标签和工具提示 -->
                    <span class="text-gray-700 tooltip-container">净现值 (NPV, 税后)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block align-text-bottom ml-1 info-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <span class="tooltip-text" style="width: 300px; margin-left: -150px;"><b>通俗解释 (税后):</b> 把未来所有(已扣税)的净节省，减去初始投资, 折算到今天看是正还是负。<b>NPV > 0 代表项目可行 (已考虑所得税及折旧税盾)。</b></span>
                    </span>
                    <span class="font-bold ${npvColor}">${fWan(boiler.npv)} 万元</span>
                </div>
                <div class="flex justify-between text-sm">
                    <!-- V11.0: 更新标签和工具提示 -->
                    <span class="text-gray-700 tooltip-container">内部收益率 (IRR, 税后)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block align-text-bottom ml-1 info-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <span class="tooltip-text" style="width: 300px; margin-left: -150px;"><b>通俗解释 (税后):</b> 这个项目本身的“年化收益率”(已考虑税收)。<b>IRR > 折现率 (基准收益率) 代表项目优秀</b>, 回报高于你的最低要求。</span>
                    </span>
                    <span class="font-bold ${irrColor}">${fPercent(boiler.irr)}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <!-- V11.0: 更新标签和工具提示 -->
                    <span class="text-gray-700 tooltip-container">动态回收期 (PBP, 税后)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block align-text-bottom ml-1 info-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <span class="tooltip-text" style="width: 300px; margin-left: -150px;"><b>通俗解释 (税后):</b> 考虑了利息、通胀和税收后, <b>“真正”需要多少年才能(在税后)收回初始投资</b>。这个值比“静态回收期”更真实、更可靠。</span>
                    </span>
...
// ... existing code ...
    if (profitableROI.length > 0) {
        ...
        // V11.0: 更新结论文本
        conclusionHTML += `<p class="text-sm text-gray-700"><b>投资回报 (ROI) 结论：</b>项目可行。相较于 <b>${profitableROI.map(p => p.name).join('、')}</b>，<b>税后IRR</b> 高于基准收益率(${fPercent(discountRate)})。回报最佳的是替代 <b>${bestIRR.name}</b>，<b>税后IRR</b> 高达 <b>${fPercent(bestIRR.irr)}</b>，<b>税后动态回收期</b> <b>${fYears(bestIRR.dynamicPBP)}年</b>。</p>`;
    } else {
         const bestNPV = comparisons.length > 0 ? comparisons.reduce((p, c) => (p.npv > c.npv) ? p : c) : null;
// ... existing code ...
         if (bestNPV && bestNPV.npv > 0) {
             // V11.0: 更新结论文本
             conclusionHTML += `<p class="text-sm text-gray-700"><b>投资回报 (ROI) 结论：</b>项目勉强可行。相较于 <b>${bestNPV.name}</b>，项目<b>税后净现值(NPV)</b>为正 (<b>${fWan(bestNPV.npv)} 万元</b>)，但所有方案<b>税后IRR</b>均未超过基准收益率(${fPercent(discountRate)})。</p>`;
         } else {
             // V11.0: 更新结论文本
             conclusionHTML += `<p class="text-sm text-red-700"><b>投资回报 (ROI) 结论：</b>项目不可行。相较于所有对比方案，项目的<b>税后IRR</b>均低于基准收益率(${fPercent(discountRate)})，且<b>税后NPV</b>均为负。</p>`;
         }
    }
...
// ... existing code ...
        ...
        <!-- V11.0: 更新标签 -->
        <h4>2. 净现值 (NPV, 税后)</h4>
        <p class="authority">NPV (Net PresentValue) 是项目全周期内产生的净现金流（税后节省 - 额外投资）按折现率折算到今天的总和。<b>NPV > 0 代表项目可行。</b></p>
        <div class="formula-block">NPV = Σ [ (PostTaxSaving_n + TaxShield_n) / (1 + r)^n ] - ΔInvest</div>
        <h4>3. 内部收益率 (IRR, 税后)</h4>
        <p class="authority">IRR (Internal Rate of Return) 是使项目净现值(NPV)等于零时的折现率。<b>IRR > 基准折现率，代表项目优秀。</b></p>
        <div class="formula-block">NPV(CashFlow_PostTax, IRR) = 0<br>其中: CashFlow = [ -ΔInvest, CF_Y1, CF_Y2, ... ]</div>
        <h4>4. 动态投资回收期 (PBP, 税后)</h4>
        <p class="authority">PBP (Payback Period) 是指考虑了资金时间价值（折现率）及税收后，项目累计节省的净现金流（折现后）等于初始额外投资所需的时间。</p>
    `;
...
// ... existing code ...
                    ...
                    <!-- V11.0: 更新标签 -->
                    <p class="pl-4 text-blue-700 font-bold">↳ LCC 节省 (税前NPV): ${fWan(boiler.lcc.total)} - ${fWan(hybridSystem.lcc.total)} = <b>${fWan(c.lccSaving)} 万元</b></p>
                    <p class="pl-4 text-blue-700 font-bold">↳ 净现值 (NPV, 税后): <b>${fWan(c.npv)} 万元</b></p>
                    <p class="pl-4 text-blue-700 font-bold">↳ 内部收益率 (IRR, 税后): <b>${(c.irr === null || !isFinite(c.irr)) ? 'N/A' : (c.irr * 100).toFixed(1) + ' %'}</b></p>
                 </div>
// ... existing code ...
        });
...
// ... existing code ...
            hp.energyCostDetails.tiers.forEach(tier => {
                hpEnergyCostDetailsHTML += `<p class="pl-4">↳ <b>${t.name}:</b> ${fNum(tier.elec, 0)} kWh * ${tier.price} 元/kWh = ${fYuan(tier.cost)} 元</p>`;
// ... existing code ...
...
// ... existing code ...
                    ...
                    <!-- V11.0: 更新标签 -->
                    <p class="pl-4 text-blue-700 font-bold">↳ LCC 节省 (税前NPV): ${fWan(boiler.lcc.total)} - ${fWan(hp.lcc.total)} = <b>${fWan(c.lccSaving)} 万元</b></p>
                    <p class="pl-4 text-blue-700 font-bold">↳ 净现值 (NPV, 税后): <b>${fWan(c.npv)} 万元</b></p>
                    <p class="pl-4 text-blue-700 font-bold">↳ 内部收益率 (IRR, 税后): <b>${(c.irr === null || !isFinite(c.irr)) ? 'N/A' : (c.irr * 100).toFixed(1) + ' %'}</b></p>
                 </div>
// ... existing code ...
...
// ... existing code ...
            ...
            <li><b>风险 (IRR/PBP误判):</b> 采用“静态回收期”替代“动态回收期(PBP)”，忽略资金的时间价值（利息/折现率）。</li>
             <!-- V11.0: 更新对策 -->
            <li><b>对策:</b> <b>(1) 明确折现率:</b> 必须与业主商定一个合理的“基准折现率/收益率”（如8%）。<b>(2) 核心指标:</b> 必须使用 <b>税后IRR</b> 和 <b>税后动态PBP</b> 作为核心财务指标。<b>(3) 税盾效应:</b> 必须考虑企业所得税及折旧带来的税盾效应。</li>
        </ul>
    `;
// ... existing code ...
}
...
// ... existing code ...
                ...
                <!-- V11.0: 新增打印行 -->
                <tr><td class="col-param">企业所得税率</td><td class="align-right">${fPercent(inputs.taxRate, 1)}</td></tr>
                <tr><td class="col-param">资产折旧年限 (年)</td><td class="align-right">${inputs.depreciationYears}</td></tr>
            </table>
        </div>
    `;
                ...
                <table class="print-report-table">
                    <thead>
                        <tr>
                            ...
                            <th class="align-right">NPV(税后, 万)</th>
                            <th class="align-right">IRR(税后, %)</th>
                            <th class="align-right">PBP(税后, 年)</th>
...
// ... existing code ...
             reportHTML += `
                <tr>
                    ...
                    <!-- V11.0: 新增税后NPV列 -->
                    <td class="align-right">${fWan(c.npv)}</td>
...
// ... existing code ...
    ...
    reportHTML += `
        <div class="print-report-footer">
            <!-- V11.0: 更新页脚 -->
            <p>注：全寿命周期成本(LCC)与ROI计算基于税后净现值(NPV)法，符合《建设项目经济评价方法与参数》相关规定。</p>
